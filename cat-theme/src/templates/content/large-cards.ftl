<section class="large-card-group" id="large-card-group">
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-md-10 large-card-group--wrapper offset-lg-0 offset-md-1">
                <div class="offset-1 caption">
                    <span class="caption--title caption--title--dot">•</span>

                    <hr class="caption--line"/>

                    <span class="caption--title">Stuff</span>
                </div>

                <h2 class="large-card-group--header">More Stuff</h2>
            </div>
        </div>
    </div>

    <div class="container-fluid">
        <div class="row">
            <div class="large-card-group--container">
                <div class="large-card">
                    <div class="large-card--overlay">
                        <a class="large-card--info">
                            <h5>Cat Fashion</h5>

                            <h4>Super Fashionable Cats</h4>

                            <span> smart and stylish </span>
                        </a>
                    </div>
                </div>

                <div class="large-card">
                    <div class="large-card--overlay">
                        <a class="large-card--info">
                            <h5>Cat Fashion</h5>

                            <h4>Super Fashionable Cats</h4>
                            
                            <span> smart and stylish </span>
                        </a>
                    </div>
                </div>

                <div class="large-card">
                    <div class="large-card--overlay">
                        <a class="large-card--info">
                            <h5>Cat Fashion</h5>

                            <h4>Super Fashionable Cats</h4>

                            <span> smart and stylish </span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
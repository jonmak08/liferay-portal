...
### Removed the liferay-ui:custom-attributes-available Tag and Replaced with liferay-frontend:custom-attributes-available
- **Date:** 2016-Jan-25
- **JIRA Ticket:** LPS-62496

#### What changed?

The `liferay-ui:custom-attributes-available` tag has been removed and replaced with the
`liferay-frontend:custom-attributes-available` tag.

#### Who is affected?

Plugins and templates that are using the `liferay-ui:custom-attributes-available` tag need to update
their usage of the tag.

#### How should I update my code?

You should import the `liferay-frontend` tag library (if necessary) and update
the tag namespace from `liferay-ui:custom-attributes-available` to `liferay-frontend:custom-attributes-available`.

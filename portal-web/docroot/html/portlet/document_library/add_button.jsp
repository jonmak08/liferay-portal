	...
	<c:choose>
		...
		<c:when test="<%= !fileEntryTypes.isEmpty() && hasAddDocumentPermission %>">
			...
			%>
				...
				<aui:nav-item href="<%= addFileEntryTypeURL %>" iconCssClass="icon-file" label="<%= HtmlUtil.escape(fileEntryType.getUnambiguousName(fileEntryTypes, themeDisplay.getScopeGroupId(), locale)) %>" localizeLabel="<%= false %>" />

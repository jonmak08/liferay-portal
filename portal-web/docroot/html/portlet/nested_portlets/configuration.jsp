	...
	<aui:fieldset label="layout-template">
		...
		%>
				...
				<td align="center" width="<%= 100 / CELLS_PER_ROW %>%">
					<img alt="<%= HtmlUtil.escapeAttribute(LanguageUtil.get(thumbnail)) %>" onclick="document.getElementById('<portlet:namespace />layoutTemplateId<%= i %>').checked = true;" src="<%= layoutTemplate.getStaticResourcePath() %><%= HtmlUtil.escapeAttribute(layoutTemplate.getThumbnailPath()) %>" /><br />

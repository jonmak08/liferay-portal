					...
					<div class="lfr-blogs-small-image-content toggler-content-collapsed">
						<aui:row>
							<c:if test="<%= smallImage && (entry != null) %>">
								<aui:col width="<%= 50 %>">
									<img alt="<%= HtmlUtil.escapeAttribute(LanguageUtil.get(preview)) %>" class="lfr-blogs-small-image-preview" src="<%= Validator.isNotNull(entry.getSmallImageURL()) ? entry.getSmallImageURL() : themeDisplay.getPathImage() + "/template?img_id=" + entry.getSmallImageId() + "&t=" + WebServerServletTokenUtil.getToken(entry.getSmallImageId()) %>" />
								</aui:col>
							</c:if>

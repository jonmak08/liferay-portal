...
<aui:input cssClass="lfr-search-keywords" id='<%= namespace + "keywords_" + StringUtil.randomId() %>' inlineField="<%= true %>" label="" name='<%= namespace + "keywords" %>' onBlur="<%= taglibOnBlur %>" onFocus="<%= taglibOnFocus %>" placeholder="<%= HtmlUtil.escape(keywords) %>" size="30" title="search-web-content" type="text" useNamespace="<%= false %>" />

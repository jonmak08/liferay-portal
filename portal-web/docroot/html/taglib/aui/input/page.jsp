...
<c:choose>
	<c:when test='<%= !type.equals("assetCategories") && !type.equals("hidden") && Validator.isNotNull(label) %>'>
		<label <%= labelTag %>>
	...
	<c:when test='<%= !type.equals("hidden") && !type.equals("image") %>'>
		<label class="hide-accessible" for="<%= forLabel %>"><liferay-ui:message key='<%= Validator.isNotNull(hideAccessibleLabel) ? hideAccessibleLabel : name %>' /></label>
	</c:when>
</c:choose>
